...
const { JWT_SECRET, SESSION_COOKIE, REFRESH_COOKIE, XSRF_HEADER_NAME } = process.env
...
class Session {
    ...
    validateXsrfToken() {
        const token = this.req.get(XSRF_HEADER_NAME)
        
        return (token && token == this.xsrf)
    }
}
...
module.exports = {
    ...
    xsrf() {
        return function(req, res, next) {
            try {
                if(!req.session.validateXsrfToken()) {
                    throw new XSRFTokenError()
                }
